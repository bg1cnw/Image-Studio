...
桌面端支持从配套的提示词聚合站 [**Image-Prompts** · prompts.sorry.ink](https://prompts.sorry.ink) 一键导入提示词。Image-Prompts 是 Image-Studio 生态里专门做「Prompt 这一侧」的中英双语提示词聚合站,提示词详情页都带「Send to Image-Studio」按钮 —— 不登录也能用,签发一次性 token 后通过自定义协议跳转到桌面端。

- macOS: 由 Wails 桌面版接收 `image-studio://import?...`
- Windows / Linux: 由 Gio 客户端接收 `image-studio://import?...`

桌面端拿到 token 后向站点回取 prompt payload(24h 过期、一次消费),弹出独立确认组件后写入当前 prompt 编辑器。仓库:[RoseKhlifa/Image-Prompts](https://github.com/RoseKhlifa/Image-Prompts)。
...
| 项目 | 角色 |
|---|---|
| [**Image-Prompts**](https://github.com/RoseKhlifa/Image-Prompts) · [prompts.sorry.ink](https://prompts.sorry.ink) | 中英双语 AI 提示词聚合站,Image-Studio 生态的「灵感源」一侧。社区投稿 + 公开档案融合,可检索 / 收藏 / 一键导入到 Image-Studio。详情页「Send to Image-Studio」会通过 `image-studio://` 深链把 prompt 直接送进本应用编辑器,**游客也可用** |
